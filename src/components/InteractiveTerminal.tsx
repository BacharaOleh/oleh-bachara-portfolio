  ...
  return (
    <div className="glass-card rounded-3xl overflow-hidden border border-white/[0.1] shadow-2xl">
      ...
      <div className="bg-slate-950/90 px-4 py-3 border-b border-white/[0.08] flex items-center justify-between flex-wrap gap-2">
        ...
        {/* Tab switcher */}
        <div className="flex items-center gap-1 bg-slate-900/80 p-1 rounded-xl border border-white/[0.06]" role="tablist" aria-label="Code files">
          {SNIPPETS.map((snippet) => (
            <button
              ...
              aria-label={`View code snippet: ${snippet.filename}`}
              onClick={() => {
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            aria-label="Run test execution for this code snippet"
            onClick={handleRun}
            disabled={running}
          ...
          <button
            ...
            onClick={handleCopy}
            className="p-1.5 rounded-lg bg-slate-900 border border-white/[0.08] text-slate-300 hover:text-white transition-colors cursor-pointer"
            title="Copy code"
          >
      ...
      {/* Snippet Header */}
      <div className="px-5 py-2.5 bg-slate-950/60 border-b border-white/[0.06] text-xs font-mono text-slate-300 flex items-center justify-between">
        <span>// {activeSnippet.description}</span>
        <span className="uppercase text-indigo-400 font-semibold">{activeSnippet.language}</span>
      </div>

      {/* Code Editor Body with Lightweight Single Pre/Code Block */}
      <div className="p-5 font-mono text-xs sm:text-sm bg-[#050810]/95 overflow-x-auto text-slate-200 leading-relaxed min-h-[200px]">
      ...
      <AnimatePresence>
        {showOutput && activeSnippet.outputSnippet && (
          ...
          >
            <div className="flex items-center gap-2 mb-1.5 text-slate-300 text-[11px] uppercase tracking-wider font-semibold">
              <Cpu size={13} className="text-emerald-400" />
              Terminal Execution Result
            </div>
            <pre className="whitespace-pre-wrap text-emerald-300 text-xs">
